...
const BOT_DIRECTORIES = ['actions', 'config', 'flows', 'entities', 'content-elements', 'intents', 'qna']
...
export class BotService {
  ...
  private async _createBotFromTemplate(botConfig: BotConfig, template: BotTemplate): Promise<BotConfig | undefined> {
    ...
    this.logger.warn(template.moduleId!)
    this.logger.warn(templatePath)
    this.logger.info(templateConfigPath)
    try {
      ...
      const newData = {
        ...
        enabled: botConfig.telegramToken ? true : false
      }
      ...
      if (fse.existsSync(templateConfigPath)) {
        ...
        this.logger.info(mergedConfigs)
        this.logger.info('==========================')
        this.logger.info(templateConfigPath)
        ...
        await scopedGhost.upsertFile('/', 'config/channel-telegram.json', stringify(newData))
